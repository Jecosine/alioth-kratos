...
func (tc *TagCreate) check() error {
	...
	if _, ok := tc.mutation.CreatedTime(); !ok {
		return &ValidationError{Name: "created_time", err: errors.New(`ent: missing required field "Tag.created_time"`)}
	}
	return nil
}
...
func (tc *TagCreate) createSpec() (*Tag, *sqlgraph.CreateSpec) {
	...
	if value, ok := tc.mutation.CreatedTime(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeTime,
			Value:  value,
			Column: tag.FieldCreatedTime,
		})
		_node.CreatedTime = value
	}
	if nodes := tc.mutation.ProblemsIDs(); len(nodes) > 0 {
