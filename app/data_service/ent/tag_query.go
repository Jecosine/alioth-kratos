...
func (tq *TagQuery) QueryProblems() *ProblemQuery {
	query := &ProblemQuery{config: tq.config}
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withProblems: tq.withProblems.Clone(),
...
func (tq *TagQuery) sqlAll(ctx context.Context) ([]*Tag, error) {
	...
	if query := tq.withProblems; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int64]*Tag, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Problems = []*Problem{}
		}
		var (
			edgeids []int64
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(tag.ProblemsPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, tq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "problems": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "problems" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Problems = append(nodes[i].Edges.Problems, n)
			}
		}
	}
