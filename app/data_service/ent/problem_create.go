...
func (pc *ProblemCreate) AddSolvedByIDs(ids ...int64) *ProblemCreate {
	pc.mutation.AddSolvedByIDs(ids...)
	return pc
}
...
func (pc *ProblemCreate) check() error {
	...
	if _, ok := pc.mutation.CreatedTime(); !ok {
		return &ValidationError{Name: "created_time", err: errors.New(`ent: missing required field "Problem.created_time"`)}
	}
	return nil
}
...
func (pc *ProblemCreate) createSpec() (*Problem, *sqlgraph.CreateSpec) {
	...
	if value, ok := pc.mutation.CreatedTime(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeTime,
			Value:  value,
			Column: problem.FieldCreatedTime,
		})
	...
	if nodes := pc.mutation.AuthorIDs(); len(nodes) > 0 {
		...
		_node.user_created_problems = &nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
	if nodes := pc.mutation.SolvedByIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
